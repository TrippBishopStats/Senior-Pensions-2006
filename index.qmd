...
## Recommendations
The recommendation from the Deputy Director of General Planning is to proceed with Option A, which utilises the existing Oportunidades programme. There are a number of reasons for this. First, while Option B (Small Towns) does reach more people, most of those additional people reached do not need the assistance. Option A is more efficient in that it reaches 80% of the eligible persons assisted by Option B but for 60% of the cost (see figures 1 & 2). This increased efficiency can be seen in figure 3 which shows that due to the lower number of ineligible people being served, the cost per eligible person is `r scales::dollar(round(total_cost_A/target_pop_served_A,0))`
 vs `r scales::dollar(round(total_cost_B/target_pop_served_B,0))`. Because of the significant different in the efficiency of the two options, the recommendation is it proceed with Option A. The savings can be used to improve the programme, particularly the number of eligible persons not covered at present. Remaining funds can then be carried over into the next year and applied to eligible persons who were not covered in the first year or the programme.  